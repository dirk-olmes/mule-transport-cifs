...
public class SmbMessageRequester extends AbstractMessageRequester {
	 ...
	 *            will be returned
	 * @return the result of the request wrapped in a MuleMessage object. Null
    ...
    protected MuleMessage doRequest(long timeout) throws Exception {
		try {
			...
			if (SmbConnector.checkNullOrBlank(uri.getUser())
				...
				logger
						.warn("No user or password supplied. Attempting to connect with just smb://<host>/<path>");
				logger.info("smb://" + uri.getHost() + uri.getPath());
				smbPath = "smb://" + uri.getHost() + uri.getPath();
			} else {
				logger.info("smb://" + uri.getUser() + ":" + uri.getPassword()
						+ "@" + uri.getHost() + uri.getPath());
				smbPath = "smb://" + uri.getUser() + ":" + uri.getPassword()
						+ "@" + uri.getHost() + uri.getPath();
			}
			...
			MuleMessage reply = new DefaultMuleMessage(connector
					.getMessageAdapter(file.getOutputStream().toString()
							.getBytes()));
			reply.setProperty(FileConnector.PROPERTY_ORIGINAL_FILENAME, file
					.getName());
			reply.setProperty(FileConnector.PROPERTY_FILE_SIZE, new Long(file
					.length()));
			return reply;
		} finally {
